...
const verificationDeadlineMs = boundedInteger("AGENT_RUN_TEST_DEADLINE_MS", 120_000, 10_000, 900_000);
const structuredOutput = process.env.AGENT_RUN_TEST_OUTPUT_SCHEMA?.trim() === "1";
const resultMarker = structuredOutput ? "STRUCTURED_READY" : "AGENT_RUN_READY";
...
const request = {
  idempotencyKey,
  message: process.env.AGENT_RUN_TEST_MESSAGE || (structuredOutput
    ? [
        "Return a structured JSON result for this verification task.",
        `Set the answer field to exactly ${resultMarker}.`,
        "Do not ask a question, request clarification, or call any tool.",
      ].join("\\n")
    : `Reply exactly ${resultMarker}. Do not ask a question, request clarification, or call any tool.`),
  ...(structuredOutput ? {
    outputSchema: {
...
const completed = await pollRun(runId, userToken, ["completed", "failed"], verificationDeadlineMs);
...
if (structuredOutput) {
  ...
  assert(completed.result?.value?.answer === resultMarker, "The structured result was not projected.");
} else {
  assert(completed.result?.kind === "text", "The text AgentRun did not return text.");
  assert(completed.result?.value === resultMarker, "The text result was not projected.");
}
...
async function pollRun(runId, token, terminalStatuses, timeoutMs) {
