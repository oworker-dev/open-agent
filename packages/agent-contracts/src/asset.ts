...
export function parseAssetPrompt(value: string): {
  readonly text: string;
  readonly assets: readonly AssetPromptReference[];
  readonly clientMessageId?: string;
} {
  const assets: AssetPromptReference[] = [];
  let clientMessageId: string | undefined;
  const text = value.replace(/^\[open-agent-(asset|message) (\{[^\n]+?\})\](?: Attached asset [^\n]*?\. Use import_asset before inspecting or processing it\.)?$/gmu, (marker, kind: string, json: string) => {
    try {
...
export function serializeAssetPrompt(text: string, assets: readonly AssetPromptReference[], clientMessageId?: string): string {
...
export interface AssetStore {
  ...
  abortUpload(input: { readonly owner: AssetOwner; readonly uploadId: string }): Promise<void>;
  deleteAsset(input: { readonly assetId: string; readonly owner: AssetOwner }): Promise<void>;
  ...
  ): Promise<AssetDownload | undefined>;
}
