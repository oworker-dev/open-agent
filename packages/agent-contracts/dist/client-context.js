...
export const AGENT_CLIENT_CONTEXT_MAX_BYTES = AGENT_CLIENT_CONTEXT_MAX_TOKENS * AGENT_APPROXIMATE_BYTES_PER_TOKEN;
/** Transport guard for model-facing client context. Semantic reduction happens before this boundary. */
export function isBoundedAgentClientContext(value) {
    if (!Array.isArray(value) || value.length > AGENT_CLIENT_CONTEXT_MAX_ENTRIES)
        return false;
    let totalBytes = 0;
    for (const entry of value) {
        ...
        if (totalBytes > AGENT_CLIENT_CONTEXT_MAX_BYTES)
            return false;
    }
    return true;
}
