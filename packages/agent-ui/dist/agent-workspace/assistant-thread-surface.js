...
function UserMessage({ messages }) {
    ...
    return (_jsxs(MessagePrimitive.Root, { className: "group relative mx-auto flex w-full max-w-(--thread-max-width) flex-col items-end", "data-latest-user": isLastUserMessage ? "" : undefined, children: [_jsx(AttachmentGroup, { className: "mb-2 max-w-[88%] justify-end py-0 empty:hidden", children: _jsx(MessagePrimitive.Attachments, { children: ({ attachment }) => _jsx(UserAttachment, { attachment: attachment, messages: messages }) }) }), _jsx("div", { className: "max-w-[min(44rem,88%)] rounded-2xl bg-muted/75 px-4 py-3 text-[15px] leading-6 text-foreground", onClick: () => {
                    if (window.matchMedia("(pointer: coarse)").matches)
                        setActionsVisible((visible) => !visible);
                }, children: _jsx(MessagePrimitive.Parts, { components: { Text: DirectiveText } }) }), _jsx("div", { className: cn("pointer-events-none absolute right-0 top-full z-10 flex h-7 items-center transition-opacity", actionsVisible ? "opacity-100" : "opacity-0 group-hover:opacity-100 group-focus-within:opacity-100"), children: _jsxs(ActionBarPrimitive.Root, { className: "pointer-events-auto flex h-7 items-center gap-0.5", children: [_jsx(ReliableCopyButton, { label: messages.copyResponse }), isLastUserMessage ? (_jsx(ActionBarPrimitive.Edit, { "aria-label": messages.editMessage, className: "inline-flex size-7 items-center justify-center rounded-md text-muted-foreground hover:bg-accent hover:text-foreground", children: _jsx(PencilIcon, { className: "size-3.5" }) })) : null] }) })] }));
}
