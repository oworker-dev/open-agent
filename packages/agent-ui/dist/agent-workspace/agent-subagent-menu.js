...
export function AgentSubagentMenu({ activeSessionId, events, durableSessions = [], locale, onControl, onOpen, }) {
    ...
    return (_jsxs(Popover, { children: [_jsx(PopoverTrigger, { asChild: true, children: _jsxs(Button, { "aria-label": localize(locale, "Open sub-agents", "打开子代理"), className: "h-8 gap-1.5 px-2.5 text-sm", size: "sm", variant: "ghost", children: [_jsx(NetworkIcon, { className: "size-4" }), _jsx("span", { className: "hidden sm:inline", children: localize(locale, "Sub-agents", "子代理") }), active.length > 0 ? (_jsx("span", { "aria-label": localize(locale, `${active.length} active`, `${active.length} 个正在执行`), className: "min-w-4 text-xs tabular-nums text-muted-foreground", children: active.length })) : null] }) }), _jsxs(PopoverContent, { align: "end", className: "w-[min(22rem,calc(100vw-1.5rem))] p-1.5", children: [_jsx(SubagentGroup, { activeSessionId: activeSessionId, label: localize(locale, "Active", "正在执行"), locale: locale, onOpen: onOpen, onControl: onControl, busySessionId: busySessionId, closedSessions: closedSessions, setBusySessionId: setBusySessionId, setClosedSessions: setClosedSessions, setStatusOverrides: setStatusOverrides, sessions: active }), _jsx(SubagentGroup, { activeSessionId: activeSessionId, label: localize(locale, "Done", "已完成"), locale: locale, onOpen: onOpen, onControl: onControl, busySessionId: busySessionId, closedSessions: closedSessions, setBusySessionId: setBusySessionId, setClosedSessions: setClosedSessions, setStatusOverrides: setStatusOverrides, sessions: done })] })] }));
}
function SubagentGroup({ activeSessionId, label, locale, onControl, busySessionId, closedSessions, setBusySessionId, setClosedSessions, setStatusOverrides, onOpen, sessions, }) {
    if (sessions.length === 0)
        return null;
