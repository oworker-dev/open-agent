import { AGENT_APPROXIMATE_BYTES_PER_TOKEN, AGENT_CLIENT_CONTEXT_MAX_ENTRIES, AGENT_CLIENT_CONTEXT_MAX_TOKENS, } from "@oworker/open-agent-contracts/client-context";
...
export function recoveryContextTokenBudget(modelContextWindowTokens) {
    ...
    const scaled = Math.floor(contextWindow * AGENT_CLIENT_CONTEXT_MAX_TOKENS /
        CODEX_REFERENCE_CONTEXT_WINDOW_TOKENS);
    return Math.max(MIN_RECOVERY_CONTEXT_TOKENS, Math.min(AGENT_CLIENT_CONTEXT_MAX_TOKENS, scaled));
}
export function interruptedTurnContextsFromEvents(events, priorContext, modelContextWindowTokens) {
    const cancelledTurnIds = events.flatMap((event) => event.type === "turn.cancelled" ? [event.data.turnId] : []);
    let context = priorContext;
    for (const turnId of cancelledTurnIds) {
        context = interruptedTurnContextFromEvents(events, turnId, context, modelContextWindowTokens);
    }
    return context;
}
export function interruptedTurnContextFromEvents(events, turnId, priorContext, modelContextWindowTokens, fallbackPrompt) {
    ...
    for (const event of events) {
        ...
        if (event.type === "message.completed" && event.data.turnId === turnId) {
            const message = event.data.message?.trim();
            if (message && !assistantMessages.includes(message))
    ...
    if (userMessages.length === 0 && fallbackPrompt?.trim())
        userMessages.push(fallbackPrompt.trim());
    ...
    const toolBudget = Math.max(256, Math.min(CODEX_TOOL_OUTPUT_TOKEN_BUDGET, Math.floor(totalBudget / 2)));
    const lines = [
        marker,
    ...
    for (const message of userMessages)
        lines.push(`Original user request: ${message}`);
    for (const message of assistantMessages)
        lines.push(`Assistant progress: ${message}`);
    for (const [callId, call] of toolCalls) {
        lines.push(formatToolCheckpoint(callId, call, toolResults.get(callId), toolBudget));
    }
    const retained = (priorContext ?? []).filter((entry) => !entry.startsWith(marker) &&
        !(turnId !== "pending" && entry.startsWith(`${INTERRUPTED_TURN_CONTEXT_PREFIX}pending]`)));
    return boundRetainedContext([...retained, lines.join("\n")], modelContextWindowTokens);
}
export function rewriteContextFromEvents(events, modelContextWindowTokens) {
    const completedTurns = new Set(events.flatMap((event) => event.type === "turn.completed" ? [event.data.turnId] : []));
    const transcript = [];
    for (const event of events) {
        ...
        if (event.type === "message.completed" &&
            ...
            const message = event.data.message?.trim();
            if (message)
                transcript.push(`Assistant: ${message}`);
        }
    }
...
export function sanitizeRetainedContext(value, modelContextWindowTokens = CODEX_REFERENCE_CONTEXT_WINDOW_TOKENS) {
...
export function boundRetainedContext(entries, modelContextWindowTokens) {
    const bounded = [];
...
export function truncateMiddleToTokenBudget(value, maxTokens) {
    ...
    if (markerBytes >= maxBytes)
        return takePrefixByUtf8Bytes(marker, maxBytes);
...
function formatToolCheckpoint(callId, call, result, tokenBudget) {
...
function payloadStatistics(value) {
...
function takePrefixByUtf8Bytes(value, maxBytes) {
    let bytes = 0;
    let end = 0;
    for (let index = 0; index < value.length; index += 1) {
        const codePoint = value.codePointAt(index) ?? 0;
...
function takeSuffixByUtf8Bytes(value, maxBytes) {
    let bytes = 0;
...
//# sourceMappingURL=retained-context.js.map