...
const schema = z.object({
  checksumSha256: z.string().regex(/^[a-f0-9]{64}$/u).optional(),
  parts: z.array(z.object({
    etag: z.string().min(1).max(258),
...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
  ...
  let input: unknown = {};
  try {
    if (Number(request.headers.get("content-length") ?? "0") > 512 * 1024) return problem(413, "asset_upload_request_too_large", "The completion request is too large.");
    const body = new TextDecoder().decode(await readBoundedRequestBody(request, 512 * 1024));
